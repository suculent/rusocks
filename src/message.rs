...
pub fn parse_connect_frame(frame: &[u8]) -> Result<ConnectMessage, String> {
    ...
    if frame[1] != BINARY_TYPE_CONNECT {
        return Err("Not a connect frame".to_string());
    }
    if let Ok(boxed) = parse_connect_message(&frame[2..]) {
        if let Ok(c) = downcast_connect(boxed) {
            return Ok(c);
        }
    }
...
pub fn parse_data_frame(frame: &[u8]) -> Result<DataMessage, String> {
    ...
    if frame[1] != BINARY_TYPE_DATA {
        return Err("Not a data frame".to_string());
    }
    if let Ok(boxed) = parse_data_message(&frame[2..]) {
        if let Ok(d) = downcast_data(boxed) {
            return Ok(d);
        }
    }
    // Fallback direct parse
...
pub fn parse_disconnect_frame(frame: &[u8]) -> Result<Uuid, String> {
    ...
    if frame[1] != BINARY_TYPE_DISCONNECT {
        return Err("Not a disconnect frame".to_string());
    }
    let payload = &frame[2..];
    if payload.len() < 16 {
...
fn downcast_connect(_m: Box<dyn Message>) -> Result<ConnectMessage, String> {
    Err("downcast not supported".to_string())
}
fn downcast_data(_m: Box<dyn Message>) -> Result<DataMessage, String> {
    Err("downcast not supported".to_string())
}
...
pub fn parse_connect_response(frame: &[u8]) -> Result<ConnectResponseMessage, String> {
    ...
    if frame[0] != PROTOCOL_VERSION || frame[1] != BINARY_TYPE_CONNECT_RESPONSE {
